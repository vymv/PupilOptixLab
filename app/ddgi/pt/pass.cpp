...
PTPass::PTPass(std::string_view name) noexcept : Pass(name) {
  ...
  m_optix_pass = std::make_unique<optix::Pass<SBTTypes, OptixLaunchParams>>(
      optix_ctx->context, m_stream->GetStream());
  InitOptixPipeline();
...
void PTPass::Run() noexcept {
  m_timer.Start();
  {
    ...
    auto &frame_buffer = util::Singleton<GuiPass>::instance()
                             ->GetCurrentRenderOutputBuffer()
                             .shared_buffer;

    if (m_show_type == 0) {  // pt
      // frame_buffer写入的内容将会被展示到gui上
      m_optix_launch_params.frame_buffer.SetData(frame_buffer.cuda_ptr,
                                                 m_output_pixel_num);
      m_optix_pass->Run(m_optix_launch_params,
                        m_optix_launch_params.config.frame.width,
      ...
      cudaMemcpyAsync(
          (void *)frame_buffer.cuda_ptr, (void *)albedo->cuda_res.ptr,
          m_output_pixel_num * sizeof(float4),
      ...
      cudaMemcpyAsync(
          (void *)frame_buffer.cuda_ptr, (void *)normal->cuda_res.ptr,
          m_output_pixel_num * sizeof(float4),
          cudaMemcpyKind::cudaMemcpyDeviceToDevice, m_stream->GetStream());
      cudaStreamSynchronize(m_stream->GetStream());
    }
...
void PTPass::SetScene(World *world) noexcept {
  ...
  m_optix_launch_params.emitters =
      world->optix_scene->emitters->GetEmitterGroup();
...
void PTPass::SetSBT(scene::Scene *scene) noexcept {
  ...
  desc.ray_gen_data = {.program_name = "__raygen__main",
  ...
  }
  {
    decltype(desc)::Pair<SBTTypes::MissDataType> miss_data = {
        .program_name = "__miss__default", .data = SBTTypes::MissDataType{}};
    desc.miss_datas.push_back(miss_data);
    decltype(desc)::Pair<SBTTypes::MissDataType> miss_shadow_data = {
        .program_name = "__miss__shadow", .data = SBTTypes::MissDataType{}};
...
void PTPass::BindingEventCallback() noexcept {
  ...
  EventBinder<ESystemEvent::SceneLoad>(
      [this](void *p) { SetScene((World *)p); });
}
...
void PTPass::Inspector() noexcept {
  ...
  ImGui::Text("Rendering average %.3lf ms/frame (%.1lf FPS)", m_time_cnt,
              1000.0f / m_time_cnt);
}
}  // namespace Pupil::ddgi::pt