...
__global__ void Test(float4 *output_image, uint2 size, unsigned int frame_cnt)
{
    ...
    float3 color = 0.5f + 0.5f * make_float3(cos(((float)pixel_x) / size.x + frame_cnt / 100.f),
                                             sin(((float)pixel_y) / size.y + frame_cnt / 100.f), 0.7f);
                  ...
                  unsigned int frame_cnt)
{
    ...
    Test<<<dim3(grid_size_x, grid_size_y), dim3(block_size_x, block_size_y), 0, stream>>>(output_image.GetDataPtr(),
                                                                                          size, frame_cnt);
}
