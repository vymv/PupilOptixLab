...
__device__ float3 sphericalFibonacci(float i, float n)
{
    ...
    float sinTheta = sqrt(1.0f - cosTheta * cosTheta < 0.0f
                              ? 0.0
                              : (1.0f - cosTheta * cosTheta > 1.0f ? 1.0 : 1.0f - cosTheta * cosTheta));
...
extern "C" __global__ void __raygen__main()
{
    ...
    float3 indirectcolor =
        ComputeIndirect(normalize(record.hit.geo.normal), record.hit.geo.position, ray_origin,
                        ...
                        optix_launch_params.probeirradiancesize, optix_launch_params.probeSideLength, 0.70f);
