...
extern "C" __global__ void __raygen__main() {
    ...
    for (auto i = 0u; i < 5; ++i) {
        ...
        if((x_i.emitter_rand.x < 0.0f) && (x_i.emitter_rand.y < 0.0f) && (x_i.emitter_rand.z < 0.0f)){
            ...
            // x_i.p_hat = optix_launch_params.reservoirs[pixel_index].y.p_hat;
            x_i.sample_type = 2;
        ...
        if(x_i.p_hat < 0.0001f)
            continue;

        //   w_mis * x_i.p_hat / x_i.p
        // = old_phat / sum_old_phat * x_i.p_hat / x_i.p
        // = old_phat / sum_old_phat * x_i.p_hat * x_i.W
        // = old_phat * x_i.p_hat * x_i.W, sum_old_phat累加，之后再除，M倍的权重
        // float w_i = old_phat * x_i.p_hat * neighbor_reservoir.W * neighbor_reservoir.M;
        float w_i = x_i.p_hat * x_i.p_hat * neighbor_reservoir.W * neighbor_reservoir.M;
        // reservoir.Update(x_i, w_i, old_phat * neighbor_reservoir.M, random);
        reservoir.Update(x_i, w_i, x_i.p_hat * neighbor_reservoir.M, random);
        M += neighbor_reservoir.M - 1;
    }
    reservoir.M = M;
    Reservoir init_reservoir = optix_launch_params.reservoirs[pixel_index];
    //   w_mis * x_i.p_hat / x_i.p
    // = init_reservoir.y.p_hat / sum_p * init_reservoir.y.p_hat * init_reservoir.W
    float w_i = init_reservoir.y.p_hat * init_reservoir.y.p_hat * init_reservoir.W * init_reservoir.M;
    // float w_i = init_reservoir.y.p_hat * init_reservoir.y.p_hat * optix_launch_params.reservoirs[pixel_index].W * init_reservoir.M;
    reservoir.Update(init_reservoir.y, w_i, init_reservoir.y.p_hat * init_reservoir.M, random);
    reservoir.M += init_reservoir.M - 1;
    reservoir.CalcMISW();
